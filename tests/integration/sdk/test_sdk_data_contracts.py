...
@pytest.mark.asyncio
async def test_create_dc_with_auto_data_product_id(sdk, dc_repo):
    # test data product id is auto generated by default
    assert sdk.auto_data_product_id_in_data_contract is True
    dp = await sdk.put_data_product({"domain": "d", "name": "n"})
    dc = await sdk.put_data_contract({}, dp_id=dp["id"])
    
    assert dc["id"] == str(sdk.id_generator.make_dc_id({**dc, "_dc_index": 0}))
    # convert custom properties array to dict, keyed by property name
    cp_dict = {prop["property"]: prop["value"] for prop in dc.get("customProperties", [])}
    assert cp_dict["dataProductId"] == dp["id"]

    # test when false dataProductId is not added
    sdk.auto_data_product_id_in_data_contract = False
    dc = await sdk.put_data_contract({}, dp_id=dp["id"])
    # convert custom properties array to dict, keyed by property name
    cp_dict = {prop["property"]: prop["value"] for prop in dc.get("customProperties", [])}
    assert "dataProductId" not in cp_dict
