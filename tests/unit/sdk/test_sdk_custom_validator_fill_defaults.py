...
def find_property_value(data, prop_name):
    from dmesh.sdk.custom_validator.preprocessor import get_custom_properties
    ...
    import os, json
    schema_path = os.path.join("examples/custom-validation/schemas/custom-properties", f"{prop_name}.json")
...
def test_enterprise_data_product_dataproduct():
    data = load_data("examples/custom-validation/dp-specs/valid/custom-odps-dataproduct-source-aligned.yaml")
    processed = preprocess(data)
    val = find_property_value(processed, "enterpriseDataProduct")
